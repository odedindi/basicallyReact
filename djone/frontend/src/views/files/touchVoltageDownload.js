...
const touchVoltageDownload = () => {

    const [email, setEmail] = useState('');
    const [ showMessage, setShowMessage ] = useState(false);

    const downloadHandler = () => {
        getFile(`${baseUrlServer}touchvoltage/download.csv`)
        .then(res => res.blob())
        .then(blob => {
    ...
    return (
        <div id="download-wrapper">
            <div id="pdf-header">
                ...
                <h1>Get touch voltage as CSV</h1>
            </div>
...
export default touchVoltageDownload;